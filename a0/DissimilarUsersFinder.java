...
/**
 * Represents the DissimilarUsersFinder, which finds and prints the most
 * dissimilar pair of users in the userUserMatrix, whose parent class is Finder.
 */
public class DissimilarUsersFinder extends Finder {
   ...
   */
  public DissimilarUsersFinder(Cfiltering cf) {
    super(cf);
  }
  ...
  public void find() {
    Printer.println(
        "\n\n\nThe most dissimilar pairs of users from above userUserMatrix "
            + "are:");
    // get the number of users
    int numOfUsers = cf.getNumOfUsers();
    // create a float to be the default smallest similarity score
    float min = matrix.getElement(0, 1);
    ...
    for (int i = 0; i < numOfUsers; i++) {
      ...
      for (int j = i; j < numOfUsers; j++) {
        ...
        if (i != j) {
          // select the smaller one between this score and the smallest we found
          min = Math.min(matrix.getElement(i, j), min);
        }
      }
    }
    // loop through the users
    for (int i = 0; i < numOfUsers; i++) {
      ...
      // which means to loop through the upper triangle of the userUserMatrix
      for (int j = i; j < numOfUsers; j++) {
        // if this score is the same as the smallest, and two users are
        // different
        if (matrix.getElement(i, j) == min && i != j) {
    ...
    // print the smallest similarity score
    Printer.printf("with similarity score of %.4f", min);
  }
